...
def itk2DImageRegistration(
        Im_arr1, Im_arr2, trans_type, metric_type, interpolator_type,
        optimizer_type):
    ...
    # Spatial Transform algorithm depend on tfunc_ which are
    # Euler,BSpline,Similarity,Affine
    if trans_type == tfunc_1:
        ...
        transform = sitk.BSplineTransformInitializer(
            fixed_image, [fixed_image.GetDimension(), 8])

    elif trans_type == tfunc_3:
        transform = sitk.CenteredTransformInitializer(
            fixed_image, moving_image, sitk.Similarity2DTransform(),
            sitk.CenteredTransformInitializerFilter.GEOMETRY)

    elif trans_type == tfunc_4:
        transform = sitk.CenteredTransformInitializer(
            fixed_image, moving_image, sitk.AffineTransform(
                fixed_image.GetDimension()),
            sitk.CenteredTransformInitializerFilter.GEOMETRY)

    # Similarity Metric algorithms depend on mfunc which are mutual
    # information, mean squares, correlation
    ...
    # Interpolator algorithm depends on ifunc which are Linear, Nearest
    # neighbor, BSpline
    ...
    # Optimizer algorithms depends on ofunc which are Conjugate Gradient Line
    # Search, LBFGSB, Powell, GradientDescen
    if optimizer_type == ofunc1:
        registration_method.SetOptimizerAsConjugateGradientLineSearch(
            learningRate=1.0, numberOfIterations=70)
        ...
        registration_method.SetOptimizerAsGradientDescent(
            learningRate=1.0, numberOfIterations=60)
...
if __name__ == '__main__':
    ...
    transform = itk2DImageRegistration(
        fixed_image_arr,
        moving_image_arr,
        'BSpline',
        'correlation',
    ...
    # combine these three scalar images into a multicomponent image named
    # img_out
